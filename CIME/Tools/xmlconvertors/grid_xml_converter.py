...
class GridmapNode(DataNode):
    ...
    def keyvalue(self):
        return "{}:{}:{}:{}".format(
            self.keys[0], self.data[self.keys[0]], self.keys[1], self.data[self.keys[1]]
        )
...
class Cime2DomainNode(DomainNode):
    ...
    def set_data(self, xmlnode):
        ...
        griddoms = self.xmlroot.findall(
            '.griddom[@grid="{}"]'.format(self.data["name"])
        )
        for gd in griddoms:
            mask = gd.get("mask")
...
class DataTree(object):
    ...
    def postprocess(self, fixlist, addlist, newxmlfile, currentxmlfile, badxmlfile):
        if len(addlist) > 0:
...
def grid_compare():
    ...
    cime5gridtree.postprocess(
        fixlist, addlist, "tempgrid.xml", cime5file, "badgrid.xml"
    )

    oklist, fixlist, addlist = diff_tree(cime2domaintree, cime5domaintree)
    cime5domaintree.postprocess(
        fixlist, addlist, "tempdomain.xml", cime5file, "baddomain.xml"
    )

    oklist, fixlist, addlist = diff_tree(cime2gridmaptree, cime5gridmaptree)
    cime5gridmaptree.postprocess(
        fixlist, addlist, "tempgridmap.xml", cime5file, "badgridmap.xml"
    )
